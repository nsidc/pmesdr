...
/*****************************************************************
  ...
  translated to C by DGL at BYU 03/01/2014 from ssmi_meta_make3.f 
   note: while fortran was well-tested, not all C options have been tested   
...
void polster1( float alon, float alat, float *x1, float *y1, float xlam, float slat)
{
/*
...
*/
   ...
   //printf("polster: %f %f %f %f %f %f\n",alon,alat,*x1,*y1,xlam,slat);
   ...
   *y1 =-(sn*cos(dtr*(sn*alon-xlam)))*rho;
   ...
   //printf("polster: %f %f %f %f %f %f %f %f\n",alon,alat,*x1,*y1,xlam,slat,sn,rho);   
...
void lambert2(float lat, float lon, float *x, float *y, 
...
/*	COMPUTE LOCAL RADIUS OF THE EARTH AT CENTER OF IMAGE */
   ...
   if (denom > 0.0) 
      ...
      fprintf(stderr,"*** Divsion error in lambert2 ***\n");
      ak = 1.0;
   }
		...
		float *lath, float *lonh, char *regname)
{
  ...
  p=getenv("SIR_region");
  if (p==NULL) p=rname; /* use default if environment variable not available */
...
int get_prompt_iarg(int *argn, char *argv[], char *prompt, int deflt)
{
...
float get_prompt_farg(int *argn, char *argv[], char *prompt, float deflt)
{
  static char line[120];
  float val=deflt;  
...
char *get_prompt_sarg(int *argn, char *argv[], char *prompt, char *deflt, char *buff, int blen)
{
  if (argv[*argn] == NULL) {
    printf("%s ",prompt);
...
int get_prompt_larg(int *argn, char *argv[], char *prompt, int deflt)
{
		      ...
		      char *mname, int F_num, char *s2g)
{
  ...
  char a_name[120], b_name[120], i_name[120], j_name[120], c_name[120], p_name[120],
    ...
    grd_iname[120], grd_jname[120], grd_pname[120], grd_cname[120], setname[120];
  ...
  dstart=get_prompt_iarg(argn,argv,"Enter starting day: ",1);
  dend=get_prompt_iarg(argn,argv,"Enter ending day: ",1);
  year=get_prompt_iarg(argn,argv,"Enter year (XXXX): ",1997);
  ...
  (void) get_prompt_sarg(argn,argv,"Enter region parameters file name: (NONE for manual input)","NONE",rfile, sizeof(rfile));
  printf("rfile=%s  %d\n",rfile,strncmp(rfile,"NONE",4));  
 
  if (strncmp(rfile,"NONE",4)==0 || strncmp(rfile,"none",4)==0) { /* use parameters file as input */
    pfile=FALSE;
    nregions=get_prompt_iarg(argn,argv,"How many regions to output? ",1);
  ...
  for (iregion=0; iregion<nregions; iregion++) {
    ...
    if (pfile) {
      ...
      regnum=get_prompt_iarg(argn,argv,"Region ID number: ",101);
    }
    ...
    if (regnum > 0) { /* use region definition from standard region definition file */
      ...
      if (get_prompt_larg(argn,argv,"Is this a polar region? (Y/N)",1)) {
	printf("Polar region:\n");
	deglat=get_prompt_farg(argn,argv,"  Degrees in latitude?",0.);
	if (get_prompt_larg(argn,argv," North pole? (Y/N)",1)) {
	  lath = 90.;	/* north pole */
	  latl = 90. - deglat;
	  lonh = 180.;
	  lonl = -180.;
	...
	poleflag=0;
	dateline=0;
	latl=get_prompt_farg(argn,argv," Lower-left corner Latitude:  ",0.);
	lonl=get_prompt_farg(argn,argv," Lower-left corner Longitude: ",0.);
      	lath=get_prompt_farg(argn,argv," Upper-right corner Latitude: ",0.);
	lonh=get_prompt_farg(argn,argv," Upper-right corner Longitude:",0.);
	dateline=get_prompt_larg(argn,argv," Region crosses daeline (Y/N) ",0);	
      }
      deglat=lath-latl;
      deglon=lonh-lonl;
    }
    ...
    printf("Projection code (0=lat/lon,1=Lambert(fixed),2=Lambert(local),5=Polar stereo,11-13=EASE): %d\n",projt);
	...
	if (poleflag==1) { /* north pole */
      ...
      lambert2(maplxlat,maplxlon,&x0,&y0,aorglat,aorglon,projt);
      lmostx = x0; /* left most x value */
      lambert2(maprxlat,maprxlon,&x0,&y0,aorglat,aorglon,projt);
      rmostx = x0; /* right most x value */
      lambert2(mapuylat,mapuylon,&x0,&y0,aorglat,aorglon,projt);
      umosty = y0; /* highest y value */
      lambert2(maplylat,maplylon,&x0,&y0,aorglat,aorglon,projt);
	...
	maplylon = xdeg;
      ...
      polster1(maplxlon,maplxlat,&x0,&y0,aorglon,aorglat);
      lmostx = x0; /* left most x value */
      printf("Polster 0 %f %f %f %f %f %f\n",maplxlon,maplxlat,x0,y0,aorglon,aorglat);
      polster1(maprxlon,maprxlat,&x0,&y0,aorglon,aorglat);
      rmostx = x0; /* right most x value */
      polster1(mapuylon,mapuylat,&x0,&y0,aorglon,aorglat);
      umosty = y0; /* highest y value */
      polster1(maplylon,maplylat,&x0,&y0,aorglon,aorglat); 
      lmosty = y0; /* lowest y */
	...
	nease=get_prompt_iarg(argn,argv,"Enter EASE grid resolution factor: (2**n * 1.5 km) ",4);
      }
    ...
    if (projt < 10) {  /* don't need this code section for EASE */
      temp = (lmostx-100.)/100.;
    ...
    if (pfile) {
      ...
      iasc=get_prompt_iarg(argn,argv,"Enter AscDesc flag (0=all,1=asc,2=dsc,3=morn,4=eve)",0);
    }
    ...
    if (pfile) {
      ...
      ibeam=get_prompt_iarg(argn,argv,"Select beam (1=19V,2=19H,3=22V,4=37V,5=37H,6=85V,7=85H) ",4);
    }
    ...
    /* SSM/I SPECIAL: reset projection size for some beams */
    if (ibeam < 6) {
      if (projt==1 || projt==2) {
	ascale = 1./8.9;    /* 8.9 km/pixel (ers-1) */
	bscale=ascale;
    ...
    if (pfile) {
      ...
      nsection=get_prompt_iarg(argn,argv,"Enter number of sections/pattern parameter (0=no sectioning) ",1);
    }
  ...
  /* save unsectioned projection info */
	...
	sprintf(setname,"%c%2s%c-%3s%0.2d-%0.3d-%0.3d.lis",sen,cegg,cpol,reg,iy,dstart,dend);
