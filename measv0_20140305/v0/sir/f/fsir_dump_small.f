...
C       GEOMETRIC TRANSFORMATIONS.  THIS VERSION READS ONLY ONE LINE OF
C       THE SIR IMAGE AT A TIME SO THAT IT DOES NOT REQUIRE AS MUCH
C       MEMORY AT ONE TIME
C
C	WRITTEN BY D.LONG: 16 Feb 2002
C
...
C
	PARAMETER (MAXSIZE=10000)       ! MAXIMUM PIXELS/LINE
...
C
C     READ ONE LINE OF IMAGE
C
           CALL READSIRF_BLOCK(IU,IERR,NHEAD,NHTYPE,IDATATYPE,STVAL,NSX,NSY,
     *           IOFF,ISCALE,SMIN,SMAX,NCNT,ANODATA,VMIN,VMAX,1,IY,NSX,IY,IREC)
	   IF (IERR.LT.0) THEN
	      PRINT *,'*** READSIRF_BLOCK READ ERROR: ',IERR
	      PRINT *,'*** PROGRAM TERMINATED'
	      STOP
	   ENDIF
C
C       COMPUTE LOCATION FOR EACH PIXEL AND WRITEOUT
C
	   DO IX=1,NSX
              X=IX+HALF
              Y=IY+HALF
     ...
     $		   IOPT,XDEG,YDEG,ASCALE,BSCALE,A0,B0)
	      WRITE(IU2,*) IX,IY,STVAL(IX),ALAT,ALON
          END DO
        END DO
	CLOSE(IU2)
	CLOSE(IU)
