...
window,xs=wx,ys=wy,30,xpos=2000,ypos=2000
...
ds=!display_size
yp=ds.y-ns-5
if (dispzoom ne 0) then window,xs=ns,ys=ns,31,xpos=10,ypos=yp,title='Zoom'
yp=yp-ns/2-35
if (dispvals ne 0) then window,xs=ns,ys=ns/2,32,xpos=10,ypos=yp,title='Pixel Parameters'
yp=yp-fix(ns*.8)-35
if (disphist ne 0) then window,xs=ns,ys=ns*.8,33,xpos=10,ypos=yp,title='Zoom Window Histogram'
   ...
   if (!Err ne 1) then begin     ; check for right click
     ...
     if ((x ne oldx)or(y ne oldy)) then begin  ; do nothing if pixel is same
       ...
       if (dispvals ne 0) then begin
         ...
         wset,32                                  ; now display the x,y,lat,lon,pixel values
         erase
       ...
       if (disphist ne 0) then begin
         wset,33                                  ; now display the histogram
...
if (dispzoom ne 0) then wdelete,31
if (dispvals ne 0) then wdelete,32
if (disphist ne 0) then wdelete,33
