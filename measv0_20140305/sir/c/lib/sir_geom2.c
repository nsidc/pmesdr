...
void ease2_map_info(int iopt, int isc, int hem, 
		    double *map_equatorial_radius_m, double *map_eccentricity, double *e2,
		    double *map_reference_latitude, double *map_reference_longitude, 
...
{ 
  int m=1;
  for (;isc>0;isc--) m *= 2;  /* compute total scale factor */

  *map_equatorial_radius_m = 6378.1370 ; /* WGS84 */
...
/*
	...
	ascale=scale multiple of base size (25.0 km/2^4)
	bscale=projection (N=1 S=-1 M==0)
...
*/
   ...
   double phi=alat*DTR;
   double lam=dlon;
   double sin_phi, q, qp, rho, x, y, epsilon;
    
   /* get base EASE2 map projection parameters */
   if (hem == 0) hem=25;  /* hem should be 25 or 32 */   
   ease2_map_info(iopt, isc, hem, &map_equatorial_radius_m, &map_eccentricity, &e2,
		  &map_reference_latitude, &map_reference_longitude, 
		  &map_second_reference_latitude, &sin_phi1, &cos_phi1, &kz,
   ...
   *thelon = (float) (r0 + ( x / map_scale )); 
   *thelat = (float) (s0 - ( y / map_scale ));
...
*/
   ...
   int hem=fix(bscale);
   int isc=fix(ascale);
   double lam, arg, phi, beta, qp, rho, x, y, e4, e6, epsilon;
   ...
   x = ((double) thelon - r0) * map_scale;
   y = (s0 - (double) thelat) * map_scale;
  
   switch (iopt) {
    case 8:   /* EASE2 grid north */
      rho = sqrt( ( x * x ) + ( y * y ) );
      arg=1.0 - ( rho * rho / ( map_equatorial_radius_m * map_equatorial_radius_m * qp ) );
      if (arg > 1.0) arg=1.0;      
      if (arg < -1.0) arg=-1.0;
      beta = asin( arg );      
      lam = atan2( x, ( -y ) );
