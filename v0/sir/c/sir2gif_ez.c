...
int main(int argc, char **argv)
{
  ...
  fprintf(stdout,"BYU SIR to GIF conversion program\n") ;
  if (argc < 3) {
    fprintf(stdout,"\nusage: %s sir_in gif_out <dmin> <dmax> <ctab>\n\n",argv[0]);
  ...
  /* use sir_ez routine to open file and read SIR header */

  sir_init_head(&head);  
  ierr = get_sir_head_name(argv[1], &head, &imf);
  if (ierr < 0) {
     fprintf(stdout,"*** Error opening/reading SIR header from file '%s'\n",argv[1]);
     exit(-1);
  }
  ...
  fprintf(stdout,"GIF Min, Max: %f , %f\n\n",smin,smax);
  ...
  /* read image data from file, converting to bytes in the process */

  bmin=0;
  bmax=255;
  
  ierr=get_sir_data_byte(imf, data, &head, smin, smax, bmin, bmax);
  if (ierr < 0)
    fprintf(stdout,"ERROR reading SIR input file\n");  
  else
    fprintf(stdout,"SIR input file successfully read\n");  
  
  /* a SIR file has its origin at the lower-left corner however a GIF
     image has its origin at the upper-left so we need to shuffle the
     rows of the byte */

  for (i = 0; i < nsy/2; i++)
    for (j = 0; j < nsx; j++) {
      temp = *(data+i*nsx+j);
      *(data+i*nsx+j) = *(data+(nsy-i-1)*nsx+j);
      *(data+(nsy-i-1)*nsx+j) = temp;
    }

  /* generate default greyscale color table for gif file*/
  ...
  ierr=writegif(argv[2], &k, data, &nsx, &nsy, rtab, gtab, btab, &j, &i);
  if (ierr < 0) {
