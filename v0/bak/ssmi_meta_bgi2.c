/* (c) copyright 2000,2013 David G. Long, Brigham Young University */
/*

   program to generate Backus-Gilbert Inversion (bgi) images from
   setup files generated by the ssmi_meta_setup program

   Written by DGL 15 July 2000
    + based on ssmi_meta_sir2.c
   Revised by DGL 23 Nov 2013 + updated to sir V3 code (see ssmi_meta_sir3.c)

*/

/* define various sirf and program parameters */

float a_init=180.0;           /* initial A value */
float meas_offset=0.0;        /* offset value */


/* the following parameters are subjectively set */
...
#define file_savings 1.00      /* measurement file savings ratio (no azimod)*/

#define REL_EOF   2           /* fseek relative to end of file */

#define VERSION 2.0			// or 2b
...
void time_updates(float pow, float ktime, float ant, int count,
...
/* begin program */  
  ...
  if (argc < 2) {
    printf("\nusage: %s setup_in gamma delta2 ithres\n\n",argv[0]);
  ...
  if (argc > 2) sscanf(argv[2],"%f",&bgi_gamma);
  if (argc > 3) sscanf(argv[3],"%f",&delta2);
  if (argc > 4) sscanf(argv[4],"%f",&ithres);

  printf("BGI options: Nsize=%d omega=%f gamma=%f delta2=%d\n",Nsize, omega, bgi_gamma, delta2);

  /* storage MUST be in ram
...
/*   read (50) irecords  ! Get number of measurement records in file */
   ...
   if (fread(&dumb,  sizeof(int)  , 1, imf) == 0) Ferror(0);  /* fortran record
								 header */
   if (fread(&irecords,sizeof(int)  , 1, imf) == 0) Ferror(1);
   if (fread(&dumb,  sizeof(int)  , 1, imf) == 0) Ferror(2); /* fortran record
								 trailer */

/*   read (50) nsx,nsy,ascale,bscale,a0,b0,xdeg,ydeg ! Get header info */

   if (fread(&dumb,  sizeof(int)  , 1, imf) == 0) Ferror(3);  /* fortran record
								 header */
   ...
   if (fread(&dumb,  sizeof(int)  , 1, imf) == 0) Ferror(12); /* fortran record
								 trailer */
...
/*   read (50) nsx2,nsy2,non_size_x,non_size_y,
   ...
   if (fread(&dumb,   sizeof(int)  , 1, imf) == 0) Ferror(43);/* fortran record
								 header */
   ...
   if (fread(&dumb,   sizeof(int)  , 1, imf) == 0) Ferror(54);/* fortran record
								 trailer */
...
*/
   ...
   do {
     ...
     if (strstr(line,"A_offset_value") != NULL) {
       x = strchr(line,'=');
       meas_offset=atof(++x);
       //printf("A_offset of %f\n",meas_offset);
     }
...
/* allocate storage space for image and working array */
  ...
  if (storage != 1) {   /* read measurement file into memory, 
    ...
    /*	   read (50,err=500,end=500) pow,ang,count,ktime,iadd,azi
...
/* generate SIR file header info */
  ...
  ndes=0;
  ispare1=0;
  strncpy(tag,"(c) 2013 BYU MERS Laboratory",40);
...
//  printf("amsr_meta_sirf2 scale factors: %d %d\n",iopt,iscale_sc);
//
  ...
  */
  ...
  v_min_E=-15.0;
  v_max_E=15.0;
  sprintf(type_E,"Sigma-0 mean error  (%s)",a_name);
...
/* Begin BGI processing */
  ...
  for (its=0; its < nsize; its++) {   /* for each pixel */
    *(a_val+its) = anodata_A;
    if ((its % nsx) == nsx/2) printf("Processing row %d of %d  %f\n",its/nsx,nsy,amin);
 ...
 /* output image file */
 sprintf(crproc,"BYU MERS:ssmi_meta_bgi v1.0 g=%f d2=%f thres=%f",bgi_gamma,delta2,ithres*wscale);
...
/* create STD and Err images */
  ...
  for (irec = 0; irec < ncnt; irec++) {
    ...
    stat_updates(pow, ang, count, (int *) store, (short int *) store2);
    store = store+4*count;
    store = store+2*count;
    if (count % 2 == 1) store=store+2;  /* ensure word boundary */
  }
done1:
  ...
  printf(" Sigma-0 STD min   max --> %f %f\n",amin,amax);
  printf(" Sigma-0 ERR min   max --> %f %f\n",bmin,bmax);

  sprintf(title,"Sigma-0 STD image of %s",regname);
  printf("Writing Sigma-0 STD (V) output SIR file '%s'\n", v_name);
  ierr = write_sir3(v_name, sxy, &nhead, nhtype, 
		    ...
		    crproc, crtime, descrip, ldes, iaopt, nia);
  if (ierr < 0) {
    fprintf(stdout,"*** ERROR writing Sigma-0 STD (V) output file ***\n");
    fprintf(stderr,"*** ERROR writing Sigma-0 STD (V) output file ***\n");
    fflush(stderr);
    errors++;
  }

  sprintf(title,"Sigma-0 err image of %s",regname);
  printf("Writing Sigma-0 err (E) output SIR file '%s' \n", e_name);
  ierr = write_sir3(e_name, sx, &nhead, nhtype, 
		    idatatype, nsx, nsy, xdeg, ydeg, ascale, bscale, a0, b0, 
		    ixdeg_off, iydeg_off, ideg_sc, iscale_sc, ia0_off, ib0_off, i0_sc,
		    ...
		    crproc, crtime, descrip, ldes, iaopt, nia);
  if (ierr < 0) {
    fprintf(stdout,"*** ERROR writing Sigma-0 err (E) output file ***\n");
    fprintf(stderr,"*** ERROR writing Sigma-0 err (E) output file ***\n");
    fflush(stderr);
    errors++;
  }
...
/* create time image */
  ...
  for (irec = 0; irec < ncnt; irec++) {
    ...
    time_updates(pow, (float) ktime, ang, count, (int *) store, (short int *) store2);
    store = store+4*count;
    store = store+2*count;
    if (count % 2 == 1) store=store+2;  /* ensure word boundary */
  }
done2:
  ...
  ierr = write_sir3(p_name, sxy, &nhead, nhtype, 
		    idatatype, nsx, nsy, xdeg, ydeg, ascale, bscale, a0, b0, 
		    ixdeg_off, iydeg_off, ideg_sc, iscale_sc, ia0_off, ib0_off, i0_sc,
...
/* create non-enhanced images */
  ...
  for (irec = 0; irec < ncnt; irec++) {
	...
	*(sxy +  iadd) = (*(sxy +  iadd) * n + pow*ang)/(n+1);	
...
done3:
  ...
  sprintf(title,"Grid Sigma-0 STD image of %s",regname);
  printf("Writing Grid Sigma-0 STD (V) output SIR file '%s'\n", grd_vname);
  ierr = write_sir3(grd_vname, sxy, &nhead, nhtype, 
		    ...
		    crproc, crtime, descrip, ldes, iaopt, nia);
  if (ierr < 0) {
    fprintf(stdout,"*** ERROR writing Grid Sigma-0 (V) STD output file ***\n");
    fprintf(stderr,"*** ERROR writing Grid Sigma-0 (V) STD output file ***\n");
    fflush(stderr);
    errors++;
  }

  /*printf("Writing Grid Istd (I) output file '%s'\n", grd_iname);
  ierr = write_sir3(grd_iname, sx2, &nhead, nhtype, 
                    idatatype, nsx2, nsy2, xdeg2, ydeg2,
		    ascale2, bscale2, a02, b02, 
		    ixdeg_off, iydeg_off, ideg_sc, iscale_sc, ia0_off, ib0_off, i0_sc,
  ...
  ierr = write_sir3(grd_jname, sx, &nhead, nhtype, 
                    idatatype, nsx2, nsy2, xdeg2, ydeg2,
		    ascale2, bscale2, a02, b02, 
		    ixdeg_off, iydeg_off, ideg_sc, iscale_sc, ia0_off, ib0_off, i0_sc,
  ...
  */
  ...
  ierr = write_sir3(grd_cname, sy, &nhead, nhtype, 
		    idatatype, nsx2, nsy2, xdeg2, ydeg2,
		    ascale2, bscale2, a02, b02, 
		    ixdeg_off, iydeg_off, ideg_sc, iscale_sc, ia0_off, ib0_off, i0_sc,
  ...
  ierr = write_sir3(grd_pname, a_temp, &nhead, nhtype, 
		    idatatype, nsx2, nsy2, xdeg2, ydeg2,
		    ascale2, bscale2, a02, b02, 
		    ixdeg_off, iydeg_off, ideg_sc, iscale_sc, ia0_off, ib0_off, i0_sc,
		    ioff_P, iscale_P, iyear, isday, ismin, ieday, iemin, 
...
/* write sir format output files */
  ...
  sprintf(title,"Non-enhanced Sigma-0 STD image of %s",regname);
  printf("Writing Non-enhanced Sigma-0 STD (V) output SIR file '%s'\n", non_vname);
  ierr = write_sir3(non_vname, sy, &nhead, nhtype, 
		    idatatype, nsx, nsy, xdeg, ydeg, 
            ascale, bscale, a0, b0, 
		    ixdeg_off, iydeg_off, ideg_sc, iscale_sc, ia0_off, ib0_off, i0_sc,
		    ioff_V, iscale_V, iyear, isday, ismin, ieday, iemin, 
		    ...
		    crproc, crtime, descrip, ldes, iaopt, nia);
  if (ierr < 0) {
    fprintf(stdout,"*** ERROR writing Non-enhanced Sigma-0 (V) STD output file ***\n");
    fprintf(stderr,"*** ERROR writing Non-enhanced Sigma-0 (V) STD output file ***\n");
    fflush(stderr);
    errors++;
  }
  fflush(stdout);
...
void time_updates(float power, float ktime, float ang, int count,
		  int fill_array[], short int response_array[])
{
  float ave;
  int i, n;
...
{  /* returns the next set of measurement from the file */
  ...
  while (flag == 1) {
    if (fread(&dumb, sizeof(int), 1, imf) != 0) {  /* fortran record header */
      /*	   read	 (50,err=500,end=500) pow,ang,count,ktime,iadd
		   read (50,err=500,end=500) (fill_array(i),i=1,count)   */
	...
	exit(-1);
      ...
      if (*pow < 300.0 && *pow > 50.0) return(0);
