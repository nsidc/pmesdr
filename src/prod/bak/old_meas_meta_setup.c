...
#define HASAZIMUTHANGLE 1   /* include azimuth angle in setup file if 1, 0 does not include az ang */
#define USE_PRECOMPUTE_FILES 1 /* use files to store precomputed locations when 1, use 0 to not use */ 
...
typedef struct { /* ssmiRSS V7 data file organization */
  int    KSAT,IORBIT,NUMSCAN;
  char   ASTART_TIME[NUMCHAR];
  double SCAN_TIME[HSCANS], ORBIT[HSCANS];
  float  SC_LAT[HSCANS], SC_LON[HSCANS], SC_ALT[HSCANS];
  int    IQUAL_FLAG[HSCANS];
  float  CEL_LAT[HI_SCAN*HSCANS], CEL_LON[HI_SCAN*HSCANS], CEL_EIA[HI_SCAN*HSCANS];
  float  CEL_AZM[HI_SCAN*HSCANS], CEL_SUN[HI_SCAN*HSCANS], CEL_LND[HI_SCAN*HSCANS];
  short int CEL_ICE[HI_SCAN*HSCANS];
  float  CEL_85V[HI_SCAN*HSCANS], CEL_85H[HI_SCAN*HSCANS];
  float  CEL_19V[ LO_SCAN*LSCANS], CEL_19H[ LO_SCAN*LSCANS];
  float  CEL_22V[ LO_SCAN*LSCANS];
  float  CEL_37V[ LO_SCAN*LSCANS], CEL_37H[ LO_SCAN*LSCANS];
...
/* BYU SSM/I approximate response */
...
void timedecode(double time, int *iyear, int *jday, int *imon, int *iday, int *ihour, int *imin, int *isec);
...
#define BOX_SIZE 30  /* defines the gain computation box (30 for lo scan SSMI) */
  ...
  label_330:; /* read next input line (file name) from meta file */   
    ...
    timedecode(d->SCAN_TIME[0],&iyear,&jday,&imon,&iday,&ihour,&imin,&isec);
    timedecode(d->SCAN_TIME[nscans-1],&iyeare,&jdaye,&imone,&idaye,&ihoure,&imine,&isece);

    printf("first scan:%d %d %d %d %d %d %d\n",iyear,iday,imon,jday,ihour,imin,isec);
    printf("last scan: %d %d %d %d %d %d %d\n",iyeare,idaye,imone,jdaye,ihoure,imine,isece);
	...
	goto label_350; /* skip to next scan if data bad */
      ...
      timedecode(d->SCAN_TIME[iscan],&iyear,&jday,&imon,&iday,&ihour,&imin,&isec);
      iday=jday;      
	...
	for (iregion=0; iregion<save_area.nregions; iregion++) { /* regions loop label_3400 */
	  ...
	  if (ibeam < 6) { /* ssmi 19..37 GHz */
	    if (i > 63) goto label_3400; /* skip non-existing low res measurements */
...
int read_ssmiRSS_TB_V07_binary(char *fname, ssmiRSSv7 *d, int verbose)
{
  ...
  if (fread(&d->ASTART_TIME, 24, 1, fid)!=1) Ferror(&ierr, 3);
  d->ASTART_TIME[24]='\0';  
...
void timedecode(double time, int *iyear, int *jday, int *imon, 
		int *iday, int *ihour, int *imin, int *isec)
{     
  ...
  if (ijd < 0) ijd=ijd-1;
  ijd0=julday(1,1,2000);   /* julian day reference */
