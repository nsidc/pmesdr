PRO loadpartsiri,filename,xll0,yll0,xur0,yur0,outarray,info,printflag,sensor,type,title,tag,cproc,ctime,des,iaopt
;
; load part of an image from a BYU SIR file format v3.0 
;  - this version INVERTS the image during read so origin is at upper left
;
...
if n_elements(filename) eq 0 then begin
   print,"SYNTAX:    loadpartsiri,filename,xll,yll,xur,yur,out,<info,pflag,sensor,type,title,tag,cproc,ctime,des,iaopt>"
   ...
   print,"      tv,order=1,bytscl(out,min=info(17),max=info(18))"
   goto, STOP
endif
...
if xll eq 0 and yll eq 0 and xur eq 0 and yur eq 0 then begin
    ...
    if xll eq 0 or yll eq 0 or xur eq 0 or yur eq 0 then begin
        print,"*** error in loadpartsiri: region corner must be in SIR 1-based convention"
        return
    endif
endelse
;
...
endif else if (iopt eq 11) or (iopt eq 12) or (iopt eq 13) then begin ; EASE 1
...
if xll eq 1 and yll eq 1 and xur eq head(0) and yur eq head(1) then begin
 ...
 ; vertically invert image
 vhalf=(head(1)-1)/2
 for j=0,vhalf do begin
     for i=0,head(0)-1 do begin
         tval=temp(i,head(1)-j-1)
         temp(i,head(1)-j-1)=temp(i,j)
         temp(i,j)=tval
     endfor
 endfor
 ...
 if idatatype eq 1 then begin             ; bytes
   ...
   for j=yll,yur do begin
     ...
     for i=0,xur-xll do begin
       temp(i,j-yll)=body(i)   ; SIR standard order
     endfor
   endfor
 ...
 for j=0,vhalf do begin
     for i=0,xur-xll do begin
         tval=temp(i,yur-yll-j)
         temp(i,yur-yll-j)=temp(i,j)
         temp(i,j)=tval
     endfor
 endfor
