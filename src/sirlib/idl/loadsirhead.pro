PRO loadsirhead,filename,info,printflag,sensor,type,title,tag,cproc,ctime,des,iaopt
;
; load an image from a BYU SIR file format v3.0
;
; written by dgl 1 Jan 2001 at BYU
;
; printflag, and the rest are optional. sensor...ctime are byte arrays
;  standard header values are printed if printflag=1
;
; note this code is written for big_endian machines (e.g., HP, SUN)
; modify openr/read statements for little_endian machines (vax,intel)
if n_elements(filename) eq 0 then begin
   print,"SYNTAX:    loadsir,filename,out,<info,pflag,sensor,type,title,tag,cproc,ctime,des,iaopt>"
   print,"  out=returned float image"
   print,"  info=projection info from file header (see code for details)"
   print,"  pflag=1 prints SIR file header"
   ...
   print,"      tv,bytscl(out,min=info(17),max=info(18))"
   goto, STOP
endif
;
...
;
; This is the description of the version 2.0 SIR file header
...
; floating point values, version 2.0 permits storage as bytes, i*2, or
...
;  head(18) is the type code (0=n/a, 1=A, 2=B, 3=Tb)
...
if N_params(0) ge 2 then info=info1
...
if N_params(0) ge 4 then sensor=string(sensor1)
...
if N_params(0) ge 5 then type=string(type1)
...
if N_params(0) ge 6 then title=string(title1)
...
if N_params(0) ge 7 then tag=string(tag1)
...
if N_params(0) ge 5 then cproc=string(cproc1)
...
if N_params(0) ge 5 then ctime=string(ctime1)
 
if N_params(0) ge 6 then begin ; read extra description string
 if head(40) gt 1 then begin
   if ldes gt 0 then des=string(des1) else des="" ; bytarr(1)
   endif else des="" ; bytarr(1)
endif

if N_params(0) ge 7 then begin ; read extra integers
  if head(40) gt 1 then begin
    if nia gt 0 then iaopt=iopt1 else iaopt=intarr(1)
  endif else iaopt=intarr(1)
endif
  
if N_params(0) ge 3 then begin ; print header info 
 if printflag eq 1 then begin
  
 print,"SIR file '",filename,"'"
 print,' Size:    ',head(0),' by ',head(1)
 ...
 if head(4) gt 16 then begin
  ...
  print,' Frequency: ',info1(25),'  Polarization: ',info1(24),' Head: ',info(33)
 endif else begin
 ...
 endif else if (iopt eq 8) or (iopt eq 9) then begin
  print," EASE2 Polar Azimuth form"
  print,"  Col, Row Scales:  ",info1(5),info1(6)
  print,"  Origin (col,row): ",info1(7),info1(8)
  print,"  Center (col,row): ",info1(3),info1(4)
 endif else if iopt eq 10 then begin
  print," EASE2 Cylindrical form"
  print,"  Col, Row Scales:  ",info1(5),info1(6)
  print,"  Origin (col,row): ",info1(7),info1(8)
  print,"  Center (col,row): ",info1(3),info1(4)
 endif else if (iopt eq 11) or (iopt eq 12) then begin
  print," EASE1 Polar Azimuth form"
  print,"  Col, Row Scales:  ",info1(5),info1(6)
  print,"  Origin (col,row): ",info1(7),info1(8)
  print,"  Center (col,row): ",info1(3),info1(4)
 endif else if iopt eq 13 then begin
  print," EASE1 Cylindrical form"
  print,"  Col, Row Scales:  ",info1(5),info1(6)
  print,"  Origin (col,row): ",info1(7),info1(8)
  print,"  Center (col,row): ",info1(3),info1(4)
 endif else print,'*** Unrecognized SIR option in loadsir'
