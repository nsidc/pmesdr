PRO loadsir,filename,outarray,info,printflag,sensor,type,title,tag,cproc,ctime,des,iaopt
;
; load an image from a BYU SIR file format v3.0
;
; written by dgl 1 Nov 2000 at BYU
; revised by dgl 7 Nov 2001 at BYU + fixed lambert scale factorprintout
; revised by dgl 9 Sep 2003 at BYU + added more comments
; revised by dgl 12 Mar 2014 at BYU + added ease2
;
...
if n_elements(filename) eq 0 then begin
   ...
   print,"  pflag=1 prints SIR file header, other values supress print"
...
if N_params(0) ge 3 then info=info1
...
if N_params(0) ge 5 then sensor=string(sensor1)
...
if N_params(0) ge 6 then type=string(type1)
...
if N_params(0) ge 7 then title=string(title1)
...
if N_params(0) ge 8 then tag=string(tag1)
...
if N_params(0) ge 6 then cproc=string(cproc1)
...
if N_params(0) ge 6 then ctime=string(ctime1)
 
if N_params(0) ge 7 then begin ; read extra description string
 if head(40) gt 1 then begin
   if ldes gt 0 then des=string(des1) else des="" ; bytarr(1)
   endif else des="" ; bytarr(1)
endif

if N_params(0) ge 8 then begin ; read extra integers
  if head(40) gt 1 then begin
