/* (c) 2002,2005,2014 BYU MERS Laboratory

   program to compute the difference between two BYU SIR file images

   Written by DGL  2 Feb. 2002
   Revised by DGL 25 Jul. 2005 + update EASE1 grid head computation
   Revised by DGL  7 Mar. 2014 + include EASE2 grid head computation
   Revised by DGL 18 Apr. 2014 + include optional header comparision, output flag
...
int check_head_elements(sir_head * head1, sir_head * head2, int prnt);

int main(int argc, char **argv)
{
  int   lx, hx, ly, hy, i, j, ix, iy;
  float nodata1, nodata2, nomdata;
  char  *in1 = NULL, *in2 = NULL, *out = NULL, *mname = NULL;
  int   ierr;
  float *stval1, *stval2, *stval, *mask;  /* pointer to image storage */
  float nodiffval = -1.e25;
  float rmin = -1.e25, rmax = 1.e25;
  int head_summary = 0;  
  int head_check = 0;  
  int same_check = 0, retval = 0; 
  float same_rms = 0.0;  
  int use_mask = 0;  
  int range_specify = 0;
  ...
  sir_head head1, head2, mhead;   /* header storage structures */
  ...
  if(argc < 2) {
    ...
    fprintf(stdout,"    -d = summarize headers [def=do not summarizer headers]\n");
    fprintf(stdout,"    -c = check header differences [def=do not summarizer headers]\n");
    fprintf(stdout,"    -l <value> = min pixel value [def=all]\n");
    fprintf(stdout,"    -h <value> = max pixel value [def=all]\n");
    fprintf(stdout,"    -m <name> = mask file name (SIR file) vals>0 counted\n");    
    fprintf(stdout,"    -o <name>  = output SIR file name for difference [def=none]\n");
    fprintf(stdout,"    -s <value> = max rms difference to be considered 'the same'\n");
  ...
  if (head_check) {      
    /* check each header element for similarity, printing out those that are different */
    /* note: projection already checked to get to here */

    retval=check_head_elements(&head1,&head2,0);
    if (retval) {
      fprintf(stdout,"Header differences:\n"); 
      retval=check_head_elements(&head1,&head2,1);
    }
  }  

  if (mname != NULL) {
    sir_init_head(&mhead);
    ierr = get_sir(mname, &mhead, &mask);
    if (ierr < 0) {
      fprintf(stdout,"*** Error mask reading SIR file '%s'\n",mname);
      exit(-1);
    }
    sir_update_head(&mhead, mask);
    nomdata = mhead.anodata + 0.0001; 
    use_mask=1;    

    /* check mask compatibility with the images, i.e. same size and projection */
    if (!(head1.nsx    == mhead.nsx    && head1.nsy    == mhead.nsy    && 
	  head1.iopt   == mhead.iopt   && 
	  head1.xdeg   == mhead.xdeg   && head1.ydeg   == mhead.ydeg   &&
	  head1.ascale == mhead.ascale && head1.bscale == mhead.bscale &&
	  head1.a0     == mhead.a0     && head1.b0     == mhead.b0) ) {
      fprintf(stdout,"*** Incompatible mask image size and/or projections\n");
  ...
  for (ix = lx; ix <= hx; ix++)
    for (iy = ly; iy <= hy; iy++) {
	 ...
	 separate counts for valid pixels.  Statistics are computed only
         for pixel values within specified range.*/

      if (!use_mask || (use_mask && *(mask+i) > nomdata && *(mask+i) > 0.5)) {
	cnt++;
  ...
  if (same_check)
    if (rms <= same_rms)	
      fprintf(stdout,"File contents 'the same' (within rms threshold of %f)\n",same_rms);
    else {
      retval=1;      
      fprintf(stdout,"File contents different with threshold %f\n",same_rms);
    }
  else
    retval=0;
...
/* support subroutine in sir_io.c to truncate off trailing spaces for printing */
extern char *strnc(char *s, char *t, int i);
...
int check_head_elements(sir_head * head1, sir_head * head2, int prnt)
{
  ...
  if (head1->nhtype != head2->nhtype) {
    if (prnt) fprintf(stdout," header types differ %d vs %d\n",head1->nhtype, head2->nhtype);
    retval=1;
  }
  
  if (strncmp(head1->title,head2->title,101) != 0) {
    if (prnt) fprintf(stdout," header title strings differ\n  %s\n  %s\n",
	    strnc(head1->title,stmp,101),strnc(head2->title,stmp,101));
    retval=1;
  }

  if (strncmp(head1->sensor,head2->sensor,41) != 0) {
    if (prnt) fprintf(stdout," header sensor strings differ\n  %s\n  %s\n",
	    strnc(head1->sensor,stmp,41),strnc(head2->sensor,stmp,41));
    retval=1;
  }
  
  if (strncmp(head1->type,head2->type,139) != 0) {
    if (prnt) fprintf(stdout," header type strings differ\n  %s\n  %s\n",
	    strnc(head1->type,stmp,139),strnc(head2->type,stmp,139));
    retval=1;
  }
  
  if (strncmp(head1->tag,head2->tag,101) != 0) {
    if (prnt) fprintf(stdout," header tag strings differ\n  %s\n  %s\n",
	    strnc(head1->tag,stmp,101),strnc(head2->tag,stmp,101));
    retval=1;
  }
  
  if (strncmp(head1->crproc,head2->crproc,101) != 0) {
    if (prnt) fprintf(stdout," header creator strings differ\n  %s\n  %s\n",
	    strnc(head1->crproc,stmp,101),strnc(head2->crproc,stmp,101));
    retval=1;
  }
  
  if (strncmp(head1->crtime,head2->crtime,29) != 0) {
    if (prnt) fprintf(stdout," header time strings differ\n  %s\n  %s\n",
	    strnc(head1->crtime,stmp,29),strnc(head2->crtime,stmp,29));
    /* retval=1; this flag is not set since we *expect* different creation times */
  }

  if (head1->ioff != head2->ioff) {
    if (prnt) fprintf(stdout," header pixel offset values differ %d vs %d\n",head1->ioff, head2->ioff);
    retval=1;
  }
  
  if (head1->iscale != head2->iscale) {
    if (prnt) fprintf(stdout," header pixel scale values differ %d vs %d\n",head1->iscale, head2->iscale);
    retval=1;
  }
  
  if (head1->anodata != head2->anodata) {
    if (prnt) fprintf(stdout," header NoData values differ %f vs %f\n",head1->anodata, head2->anodata);
    retval=1;
  }
  
  if (head1->iyear != head2->iyear) {
    if (prnt) fprintf(stdout," header year values differ %d vs %d\n",head1->iyear, head2->iyear);
    retval=1;
  }
  
  if (head1->isday != head2->isday) {
    if (prnt) fprintf(stdout," header start day values differ %d vs %d\n",head1->isday, head2->isday);
    retval=1;
  }
  
  if (head1->ieday != head2->ieday) {
    if (prnt) fprintf(stdout," header end day values differ %d vs %d\n",head1->ieday, head2->ieday);
    retval=1;
  }
  
  if (head1->ismin != head2->ismin) {
    if (prnt) fprintf(stdout," header start minute values differ %d vs %d\n",head1->ismin, head2->ismin);
    retval=1;
  }
  
  if (head1->iemin != head2->iemin) {
    if (prnt) fprintf(stdout," header end minute values differ %d vs %d\n",head1->iemin, head2->iemin);
    retval=1;
  }
  
  if (head1->iregion != head2->iregion) {
    if (prnt) fprintf(stdout," header region codes differ %d vs %d\n",head1->iregion, head2->iregion);
    retval=1;
  }
  
  if (head1->itype != head2->itype) {
    if (prnt) fprintf(stdout," header file type codes differ %d vs %d\n",head1->itype, head2->itype);
    retval=1;
  }
  
  if (head1->ipol != head2->ipol) {
    if (prnt) fprintf(stdout," header polarization differ %d vs %d\n",head1->ipol, head2->ipol);
    retval=1;
  }
  
  if (head1->ifreqhm != head2->ifreqhm) {
    if (prnt) fprintf(stdout," header frequency values differ %d vs %d\n",head1->ifreqhm, head2->ifreqhm);
    retval=1;
  }
  
  if (head1->idatatype != head2->idatatype) {
    if (prnt) fprintf(stdout," header data type codes differ %d vs %d\n",head1->idatatype, head2->idatatype);
    retval=1;
  }
  
  if (head1->nhead != head2->nhead) {
    if (prnt) fprintf(stdout," header nhead values differ %d vs %d\n",head1->nhead, head2->nhead);
    retval=1;
  }
  
  if (head1->v_min != head2->v_min) {
    if (prnt) fprintf(stdout," header Visiblity_min values differ %f vs %f\n",head1->v_min, head2->v_min);
    retval=1;
  }
  
  if (head1->v_max != head2->v_max) {
    if (prnt) fprintf(stdout," header Visiblity_max values differ %f vs %f\n",head1->v_max, head2->v_max);
    retval=1;
  }
  
  if (head1->ldes != head2->ldes) {
    if (prnt) fprintf(stdout," header DESCRIP cnts differ %d vs %d (des not checked)\n",head1->ldes, head2->ldes);
    retval=1;
  }
  
  if (head1->nia != head2->nia) {
    if (prnt) fprintf(stdout," header NIAs  differ %d vs %d (extra ints not checked)\n",head1->nia, head2->nia);
    retval=1;
  }
