...
C
C       Revised 12 Mar 2014 + added EASE2
C
...
C
	...
	ELSE IF (IOPT.EQ.8.OR.IOPT.EQ.9) THEN ! EASE2 NORTH OR SOUTH 
	 WRITE (*,*) 'EASE2 polar azimuthal form: ',ioff,iscale,iopt
	 WRITE (*,*) ' Nscale, Etype:',ascale,bscale
	 WRITE (*,*) ' Map origin (col/row):',a0,b0
	 WRITE (*,*) ' Map center (col/row):',xdeg,ydeg
	ELSE IF (IOPT.EQ.10) THEN	! EASE2 CYLINDRICAL
	 WRITE (*,*) 'EASE2 cylindrical form: ',ioff,iscale,iopt
	 WRITE (*,*) ' Nscale, Etype:',ascale,bscale
	 WRITE (*,*) ' Map origin (col/row):',a0,b0
	 WRITE (*,*) ' Map center (col/row):',xdeg,ydeg
	ELSE IF (IOPT.EQ.11.OR.IOPT.EQ.12) THEN	! EASE1 NORTH OR SOUTH
	 WRITE (*,*) 'EASE1 polar azimuthal form: ',ioff,iscale,iopt
	 WRITE (*,*) ' A, B scales:',ascale,bscale
	 WRITE (*,*) ' Map origin (col/row):',a0,b0
	 WRITE (*,*) ' Map center (col/row):',xdeg,ydeg
	ELSE IF (IOPT.EQ.13) THEN	! EASE1 CYLINDRICAL
	 WRITE (*,*) 'EASE1 cylindrical form: ',ioff,iscale,iopt
	 WRITE (*,*) ' A, B scales:',ascale,bscale
	 WRITE (*,*) ' Map origin (col/row):',a0,b0
	 WRITE (*,*) ' Map center (col/row):',xdeg,ydeg
