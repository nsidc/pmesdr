/* this files contains generic C code to write BMP and GIF files */
...
/*
   program to read BYU SIR image files and write out as .bmp file

   Written by DGL Aug. 8, 1997

   Written in ANSI C.  Define or undefine SWAP based on machine byte order
...
static void putlongword(w, fp)
int w;
FILE *fp;
{
  /* writes a 32-bit integer in GIF order (LSB first) */
  fputc(w & 0xff, fp);
  fputc((w>>8)&0xff, fp);
  fputc((w>>16)&0xff, fp);
  fputc((w>>24)&0xff, fp);
	     ...
	     char *rmap, char *gmap, char *bmap, int *numcols)
{
  ...
  if (fwrite("BM",sizeof(char),2,fp) != 2) {
    fclose(fp);
    return(-1);
  }

  len=256*4+54+ *nh * *nw;  /* length of file */
  putlongword(len, fp);

  len=0;
  putlongword(len, fp);

  len=54+256*4;    /* offset of data in file */
  putlongword(len, fp);

  len=40;    /* length of bitmap header */
  putlongword(len, fp);
  ...
  slen=1;  /* planes */
  putshortword(slen, fp);

  slen=8;  /* bits per pixel */
  putshortword(slen, fp);
  
  len=0;  /* compression */
  putlongword(len, fp);
  
  len=*nw * *nh;  /* size of compressed image */
  putlongword(len, fp);

  len=2925;  /* default horizontal pixels/meter */
  putlongword(len, fp);

  len=256;  /* colors used*/
  putlongword(len, fp);

  len=256;  /* important colors*/
  putlongword(len, fp);

  for (i=0; i<256; i++) {  /* color map */
    if (fwrite(&bmap[i],sizeof(char),1,fp) != 1) {
      fclose(fp);
      return(-1);
    }
    if (fwrite(&gmap[i],sizeof(char),1,fp) != 1) {
      fclose(fp);
      return(-1);
    }
    if (fwrite(&rmap[i],sizeof(char),1,fp) != 1) {
      fclose(fp);
      return(-1);
    }
    c='\0';
  ...
  c=0;
  for (len=0; len < *nh; len++) {
    if (fwrite((image + *nw * len),sizeof(char), *nw,fp) != *nw) {
      fclose(fp);
      return(-1);
    }

    if ((*nw % 2) == 1) {  /* add padding if required */
      if (fwrite(&c,sizeof(char),1,fp) != 1) { 
	fclose(fp);
	return(-1);
      }
