/********************************************************************************
 ...
 code to do plotting to a byte image - used in conjunction w/ ll_nr_fp4.c
...
/* 	COL	(I)	VALUE (COLOR) */
	...
	if (inv) {
		...
		*((short int *) (fpimage_1.a+fpimage_1.bytesperpixel*((x + iyy) * ixw + y))) = *col;
		break;
	      case 3:
		fpimage_1.a[3*((x + iyy) * ixw + y)] = fpimage_1.pal[*col];
		fpimage_1.a[3*((x + iyy) * ixw + y)+1] = fpimage_1.pal[*col+256];
		fpimage_1.a[3*((x + iyy) * ixw + y)+2] = fpimage_1.pal[*col+512];
		break;		
	      case 4:
		...
		*((short int *) (fpimage_1.a+fpimage_1.bytesperpixel*(x + (y + iyy) * ixw))) = *col;
		break;
	      case 3:
		fpimage_1.a[3*(x + (y + iyy) * ixw)] = fpimage_1.pal[*col];
		fpimage_1.a[3*(x + (y + iyy) * ixw)+1] = fpimage_1.pal[*col+256];
		fpimage_1.a[3*(x + (y + iyy) * ixw)+2] = fpimage_1.pal[*col+512];
		break;		
...
/* 	IERR	(I)	NOT USED */
	...
	*((short int *) (fpimage_1.a + index * fpimage_1.bytesperpixel)) = *val;
	break;
      case 3:
	fpimage_1.a[3*index] = fpimage_1.pal[*val];
	fpimage_1.a[3*index+1] = fpimage_1.pal[*val+256];
	fpimage_1.a[3*index+2] = fpimage_1.pal[*val+512];
	break;		
      case 4:
...
int fpimage_color(int c_opt, unsigned char *red, unsigned char *blue, unsigned char *green)
{
  switch(fpimage_1.bytesperpixel) {  
  case 1:  /* 8 bits */
