// sir_cpp BYU SIR file format C++ include file
//
// This include file defines a C++ programing interface for the BYU SIR
...
// sirheader must beidentical to sir_head defined in sir_ez.h
...
/* function prototypes (from sir_ez.c) */

void sir_init_head(struct sirheader *head);
/* sets optional parameter flags to their default values.
   call before using struct sirheader */

int get_sir_head_file(FILE *imf, struct sirheader *head);
/* read sir file header given file pointer. Returns negative value
   on read error. */

int get_sir_head_name(char *fname, struct sirheader *head, FILE **imf);
/* read sir file header given file name.  Returns negative value
   on read/open error. */

float *sir_data_alloc(struct sirheader *head);
/* returns a pointer for a malloc'd storage array sized for storage
   of the sir image data */

int get_sir_data(FILE *imf, float *stval, struct sirheader *head);
...
int get_sir_data_block(FILE *imf, float *stval, struct sirheader *head,
...
int get_sir_data_byte(FILE *imf, char *stval, struct sirheader *head, float smin,
...
int get_sir(char *fname, struct sirheader *head, float **stval);
...
void print_sir_head(FILE *omf, struct sirheader *head);
/* summary print sir file header information to file pointer */

int put_sir(char *fname, struct sirheader *head, float *stval);
/* write sir file header and data to file.  Returns negative value
   on write error. */

int sir_latlon2pix(float alon, float alat, float *x, float *y, struct sirheader *head);
...
void sir_pix2latlon(float x, float y, float *alon, float *alat, struct sirheader *head);
/* compute lat, lon from pixel location.  Will work for values outside of 
   image for most projections */

int sir_pix(float x, float y, int *ix, int *iy, struct sirheader *head);
...
int sir_lex(int ix, int iy, struct sirheader *head);
...
int isir_lex(int *ix, int *iy, int i, struct sirheader *head);
...
void sir_update_head(struct sirheader *head, float *stval);
